...
class IngresoProducto(Base):
    ...
    id_orden_compra = Column(BigInteger, ForeignKey('orden_de_compra.id_orden'), nullable=False)
...
class IngresoProductoDetalle(Base):
    __tablename__ = 'ingresos_productos_detalle'

    id_ingreso_detalle = Column(BigInteger, primary_key=True, autoincrement=True)
    id_ingreso = Column(BigInteger, ForeignKey('ingresos_productos.id_ingreso'), nullable=False)
    id_insumo = Column(BigInteger, ForeignKey('insumo.id_insumo'), nullable=False)
    cantidad_ordenada = Column(DECIMAL(12, 4), nullable=False)
